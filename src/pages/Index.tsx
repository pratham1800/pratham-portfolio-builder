          ...
          {projects.map((p, i) => (
            <ScrollFadeIn key={p.title} delay={i * 0.1}>
              <Link to={p.to}>
                ...
                >
                  <div className={`w-full bg-secondary flex items-center justify-center overflow-hidden ${p.imageFirst ? "md:order-1" : "md:order-2"}`}>
                    <img src={p.preview} alt={`${p.title} preview`} className="w-full h-auto object-contain group-hover:scale-105 transition-transform duration-500" />
                  </div>
